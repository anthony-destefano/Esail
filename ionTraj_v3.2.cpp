...
const double PI = 3.1415926535897932384626433832795; // https://www.piday.org/million/
...
inline double thermalSpeed(double T, double m) { return sqrt(2. * BOLTZMANN_CONSTANT * T / m); }

// helper functions
void maxwellianRand(double& vx, double& vy, double vth)
{
	double R, TH;
	...
	do{
		...
		vx = R * cos(TH) * vth / sqrt(2.);
		vy = R * sin(TH) * vth / sqrt(2.);
	} while(isnan(R) || isnan(TH) || isinf(vx) || isinf(vy));
}
...
struct codeOptions
{
	...
	int numberSampleThermalDistribution; // the number of samples of the thermal distribution per track. For cold beam, use 1 and temp = 0.
};

void init_codeOptions(codeOptions& co, double sfeed, double toeef, double teep, double tfep, double dtbp, double eeprk,
	double eeprm, int dmd, int mrd1, int mrd2, int nstd)
{
	...
	co.numberSampleThermalDistribution = nstd;
	...
	if(VERBOSE_INIT){
		...
		cout << "  numberSampleThermalDistribution  = " << nstd << " \n\n";
	}
	// if(HELP_INIT) {
...
{
	int N = p.codeOptionsP->numberSampleThermalDistribution;
	double ux_th, vy_th, vth;
	double Fxn = 0.0, Fyn = 0.0, dpxn = 0.0, dpyn = 0.0;
	vth = thermalSpeed(p.ionP->temperature, p.ionP->mass);

	for (int i = 0; i < N; ++i)
	{
		if(vth > 0)
			maxwellianRand(ux_th, vy_th, vth);
		//cout << ux_th << ' ' << vy_th << endl;

		init_trackVars(t,
		/* x0 */       x0,
		/* y0 */       y0,
		/* u0 */       p.ionP->speed + ux_th,
		/* v0 */       vy_th,
...
void forceOnTetherRomberg(paramList& p)
{
	...
	if(PRINT_FORCE == 1) {
		...
		cout << Kelvin_to_eV(p.ionP->temperature) << ' ';  // eV, ion thermal energy
		cout << p.tetherP->potential << ' '; // eV, tether energy
		cout << 1000 * sqrt(sqr(FxFinal) + sqr(FyFinal)) / (p.ionP->density * p.ionP->mass * sqr(p.ionP->speed)) << ' '; // mm, stopping distance
		...
		//cout << -FyFinal * 1.E9 << endl; // nN/m, net force in y direction
	}
...
void forceOnTether(paramList& p) // obsolete
{
	int N = 100;//200000;
	double lambdaD = p.electronP->debyeLength;
...
int main(int argc, char const *argv[])
{
	...
	double Tp_min = 1e-1; // eV
	double Tp_max = 1e2;  // eV
