...
#define PRINT_GRID 1 // 0 = no output, 1 = print to file
// SI units
...
// cell system for charge density calculation
struct CellSystem
{
	int XDimension;
	int YDimension;
	double BoundaryScale;
	std::map<pair<int, int>, double> cs; 
};

void print_CellSystem(CellSystem& CellStruct,string FileName)
{
	ofstream vtFile(FileName.c_str());
	for(int j = 0; j< CellStruct.YDimension; j++)
	{
		for(int i = 0; i< CellStruct.XDimension; i++)
		{
			vtFile << CellStruct.cs[{i,j}] << " ";
		} 
		vtFile << '\n';
	}
	vtFile.close();
...
void zero_CellSystem(CellSystem& CellStruct)
{
	for(int i = 0; i< CellStruct.XDimension; i++)
	{
		for(int j = 0; j< CellStruct.YDimension; j++)
...
int GetCellIndex(double x, int xd, double bl)
{
	return int(((x + bl)/(2.0*bl))*xd);
}

//Cell System Constructor
void init_CellSystem(CellSystem& CellStruct, int XD0, int YD0, double B)
{
	CellStruct.BoundaryScale = B;
	CellStruct.XDimension = XD0;
    CellStruct.YDimension = YD0;
    for(int i = 0; i< CellStruct.XDimension; i++)
	{
		for(int j = 0; j< CellStruct.YDimension; j++)
		{
			CellStruct.cs.insert({{i,j},0});
		} 
	}
}
	                  ...
	                  paramList& p)
{
    ...
    /* find cell based off position for charge density
    Xcord = int(((t.xi + boundaryLength)/(2*boundaryLength))*XDimensions);
    Ycord = int(((t.yi + boundaryLength)/(2*boundaryLength))*YDimensions);
    */
...
int main(int argc, char const *argv[])
{
	...
	paramerters.gridCellsP = &gridCells;
	...
	for (k = 0; k < P; ++k)
	{
		for (j = 0; j < M; ++j)
		{
			...
			{	
				...
				if(PRINT_GRID)
					print_CellSystem(gridCells,"GridCells.txt");
				return 0;		 	
			 }
		 	// if(PRINT_FORCE){
				// 	cout << endl << endl;
